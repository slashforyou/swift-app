# Step 19: Passer a l'etape Schedule
# Clique sur Next pour valider les adresses
...
Write-Step 19 "Passage a l'etape Schedule"
...
if ($nextBtn.bounds -match "\[(\d+),(\d+)\]\[(\d+),(\d+)\]") {
    ...
    # Verifier qu'on est sur l'etape Schedule
    $ui = Get-Screen
    $scheduleFound = $ui.SelectSingleNode("//node[contains(@text,'Schedule') or contains(@text,'Pickup Time')]")
    if ($scheduleFound) {
        Write-OK "Etape Schedule atteinte"
        return @{ Success = $true }
    } else {
        Write-FAIL "Etape Schedule non detectee"
        return @{ Success = $false; Error = "Schedule step not reached" }
    }
}
