...
describe("CRUD - Jobs", () => {
  ...
  describe("job_validation", () => {
    it("should validate time window (start before end)", () => {
      ...
      expect(
        isValidTimeWindow("2026-01-15T08:00:00Z", "2026-01-15T10:00:00Z"),
      ).toBe(true);
      expect(
        isValidTimeWindow("2026-01-15T10:00:00Z", "2026-01-15T08:00:00Z"),
      ).toBe(false);
    });
    ...
    it("should validate at least one address is provided", () => {
      const hasValidAddresses = (addresses: any[]): boolean => {
        return (
          addresses.length > 0 && addresses.every((a) => a.street && a.city)
        );
      };
