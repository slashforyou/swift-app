#!/usr/bin/env python3
"""
Deploy Gamification V2 Quest System — Phase 2c
1. Create `quests` catalog table with seed data
2. Verify gamification_quest_progress is compatible
"""
import subprocess
...
# Write SQL to temp file on server then execute
sql_escaped = SQL.replace("'", "'\\''")  # escape for bash

# Use a temp file approach
import tempfile, os

# Write SQL to local temp file then scp
tmp_path = '/tmp/gamif_quests_migration.sql'
with open('gamif_quests_migration.sql', 'w', encoding='utf-8') as f:
    f.write(SQL)

# SCP to server
result = subprocess.run(
    ['scp', 'gamif_quests_migration.sql', f'{SERVER}:/tmp/gamif_quests_migration.sql'],
...
# Execute on server
result = subprocess.run(
    ['ssh', SERVER, f'mysql -u {DB_USER} -p{DB_PASS} {DB_NAME} < /tmp/gamif_quests_migration.sql'],
    capture_output=True, text=True
)
print("MySQL exit code:", result.returncode)
print("Output:", result.stdout[:500])
if result.stderr:
    # Filter benign warning
