#!/usr/bin/env python3
"""
Fix gamificationV2.js (wrong pattern) and index.js (wrong injection).
Rewrite using the correct pattern: connect() from swiftDb, exported functions.
"""
...
# ─────────────────────────────────────────────────────────────────
# STEP 1 – Rewrite gamificationV2.js with correct pattern
# ─────────────────────────────────────────────────────────────────
print("\n=== STEP 1 – Rewrite gamificationV2.js ===")
api_js = r"""'use strict';
...
const getV2ProfileEndpoint = async (req, res) => {
  ...
  try {
    ...
    const profile = rows[0] || {
      ...
      jobs_completed_count: 0, photos_uploaded_count: 0
    };
...
// ─── GET /swift-app/v1/user/gamification/v2/leaderboard?period=weekly&page=1 ─
...
// ─── GET /swift-app/v1/user/gamification/v2/history?page=1 ──────────────────
...
# ─────────────────────────────────────────────────────────────────
# STEP 2 – Fix index.js: remove stray require, add proper routes
# ─────────────────────────────────────────────────────────────────
print("\n=== STEP 2 – Fix index.js (routes) ===")
index = read_remote(f"{APP_DIR}/index.js")

# Remove the stray bare require injected by previous script
stray = "const gamificationV2 = require('./endPoints/v1/gamificationV2');\n"
if stray in index:
    index = index.replace(stray, "", 1)
    print("Removed stray require")

# Check if V2 routes already properly registered
if "gamification/v2/profile" in index:
    print("V2 routes already registered")
else:
    # Inject after the last admin gamification route block (after refresh-cache route)
    target = """app.post('/swift-app/v1/admin/gamification/refresh-cache', (req, res) => {
  ...
  refreshCacheEndpoint(req, res);
    ...
    new_routes = """app.post('/swift-app/v1/admin/gamification/refresh-cache', (req, res) => {
  console.log('[ Admin Refresh Gamification Cache endpoint called ]');
  const { refreshCacheEndpoint } = require('./endPoints/v1/gamificationAdmin');
  refreshCacheEndpoint(req, res);
});
...
app.get('/swift-app/v1/user/gamification/v2/history', authenticateToken, (req, res) => {
  ...
  getV2HistoryEndpoint(req, res);
    ...
    if target in index:
        index = index.replace(target, new_routes, 1)
        print("Routes V2 injected after refresh-cache route")
    else:
        print("WARNING: refresh-cache pattern not found, trying fallback injection")
        # Fallback: inject after last /swift-app/v1/user/gamification route
        all_gamif = list(re.finditer(r"app\.[a-z]+\('/swift-app/v1/user/gamification[^']*'[^\n]+\n(?:[^\n]+\n)*?\}\);", index))
        if all_gamif:
            last = all_gamif[-1]
            gamif_routes = """
...
"""
            index = index[:last.end()] + gamif_routes + index[last.end():]
            print("Fallback: Routes V2 injected after last gamification route")
        else:
            print("ERROR: Could not find any gamification routes in index.js!")
...
# ─────────────────────────────────────────────────────────────────
# STEP 3 – PM2 restart + verify no errors
# ─────────────────────────────────────────────────────────────────
print("\n=== STEP 3 – PM2 restart ===")
ssh("pm2 restart swiftapp && sleep 3 && pm2 logs swiftapp --lines 15 --nostream")
