...
export default function CalendarNavigation({
  ...
  const authCheck = useAuthCheck(
    navigation,
    t("calendar.navigation.loadingCalendar"),
  );
  ...
  if (authCheck.error) {
    return (
      <View style={commonStyles.containerCentered}>
        ...
        >
          {t("calendar.navigation.authenticationError")}: {authCheck.error}
        </Text>
        <TouchableOpacity
          style={[commonStyles.buttonPrimary, commonStyles.marginTop]}
          onPress={() => navigation.navigate("Connection" as any)}
        >
          <Text style={commonStyles.buttonPrimaryText}>
            {t("calendar.navigation.goToLogin")}
          </Text>
        </TouchableOpacity>
      </View>
    );
    ...
    <View style={commonStyles.container}>
      ...
      {calendar.error && (
        <View style={customStyles.errorBanner}>
          <View style={customStyles.bannerContent}>
            <Text style={[commonStyles.body, { flex: 1, marginRight: 8 }]}>
              {calendar.error}
            </Text>
            <TouchableOpacity
          ...
          >
            <Text style={[commonStyles.h3, { color: colors.background }]}>
              ×
            </Text>
          </TouchableOpacity>
        </View>
      ...
      >
        ...
        <CalendarStack.Screen
          ...
          component={DayScreen}
          initialParams={
            initialRouteName === "Day" ? (initialParams as any) : undefined
          }
          options={{
