...
const StripeAccountStatus: React.FC<StripeAccountStatusProps> = ({
  ...
  const getStatusText = (status: string) => {
    switch (status) {
      case "complete":
        return t("stripe.accountStatus.verified");
      case "pending":
        return t("stripe.accountStatus.pending");
      case "restricted":
        return t("stripe.accountStatus.restricted");
      case "rejected":
        return t("stripe.accountStatus.rejected");
      default:
        return t("stripe.accountStatus.unknown");
    }
  };
    ...
    >
      <View style={{ marginBottom: DESIGN_TOKENS.spacing.lg }}>
        ...
        >
          ...
          {onRefresh && (
            <Button
              title={t("stripe.accountStatus.refresh")}
              variant="outline"
              size="sm"
              onPress={onRefresh}
            />
      ...
      {hasRequirements && (
        ...
        >
          ...
          >
            {t("stripe.accountStatus.actionsRequired", {
      ...
      {(hasRequirements || account.verification_status !== "complete") &&
        onCompleteOnboarding && (
          <Button
            title={
              hasRequirements
                ? t("stripe.accountStatus.completeInfo")
                : t("stripe.accountStatus.finalizeVerification")
            }
            variant="primary"
      ...
      >
        ...
        {account.created && (
          ...
          >
            Créé le:{" "}
            {formatDate(new Date(account.created * 1000), currentLanguage)}
          </Text>
        )}
      </View>
    </Card>
