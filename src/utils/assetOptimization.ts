 ...
 */
/**
 * Optional imgix CDN base URL.
 * When set, `getOptimizedImageUrl` rewrites storage URLs to this CDN.
 *
 * Format: "https://your-subdomain.imgix.net"  (no trailing slash)
 * Leave as empty string to disable CDN rewriting (returns original URL).
 *
 * @example "https://swift.imgix.net"
 */
export const CDN_IMAGE_BASE_URL: string = "";

/** Storage host patterns that should be rewritten when CDN is active */
const STORAGE_HOSTS = ["storage.googleapis.com", "storage.cloud.google.com"];
...
export function getOptimizedImageUrl(
  ...
  try {
    ...
    // If an imgix CDN base URL is configured and the image lives on a
    // known storage host, rewrite to imgix fetch-mode URL.
    if (CDN_IMAGE_BASE_URL && STORAGE_HOSTS.includes(parsed.hostname)) {
      const imgixUrl = new URL(
        `${CDN_IMAGE_BASE_URL}/${encodeURIComponent(baseUrl)}`,
      );
      imgixUrl.searchParams.set("w", String(width));
      if (height) imgixUrl.searchParams.set("h", String(height));
      imgixUrl.searchParams.set("auto", "format,compress");
      imgixUrl.searchParams.set("fit", "max");
      return imgixUrl.toString();
    }

    // For non-CDN storage URLs, append width/height hints so a future
    // server-side proxy or CDN can pick them up without breaking anything.
