...
export function validateJobStep(job: any, timeline?: any[]): StepValidationResult {
  ...
  console.log('🔍 [STEP VALIDATOR] Validating job step:', {
    jobId: job?.id,
    currentStep,
    status,
    totalSteps
...
export async function correctJobStep(
  ...
  try {
    console.log(`🔧 [STEP VALIDATOR] Correcting job ${jobCode} step from ${validation.currentStep} to ${validation.expectedStep}`);
    
    // Appeler l'API pour corriger le step
    const result = await updateJobStep(jobCode, validation.expectedStep);
    
    console.log(`✅ [STEP VALIDATOR] Step corrected successfully:`, result);
...
export async function validateAndCorrectJobStep(
  ...
  console.log('🔍 [STEP VALIDATOR] Validation result:', validation);
